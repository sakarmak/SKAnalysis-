...
class HPSTauHLTStudiesAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      edm::EDGetTokenT<reco::JetTagCollection> l2p5TauJetIsoOfflineNew3Token_;
...
HPSTauHLTStudiesAnalyzer::HPSTauHLTStudiesAnalyzer(const edm::ParameterSet& iConfig) :
    ...
    l2p5TauJetIsoOfflineNew3Token_(consumes<reco::JetTagCollection>(iConfig.getParameter<edm::InputTag>("l2p5IsoOfflineNew3"))),
    ...
    hltBeamSpotToken_(consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("beamSpot")))
{
   ...
   tree2->Branch("l2p5TauDZ",&l2p5TauDZ,"l2p5TauDZ/F");
   tree2->Branch("l2p5TrackPt",&l2p5TrackPt,"l2p5TrackPt/F");
   tree2->Branch("l2p5TauDxy",&l2p5TauDxy,"l2p5TauDxy");
   ...
   tree->Branch("l2p5TauOfflineIsoNew3",&l2p5TauOfflineIsoNew3,"l2p5TauOfflineIsoNew3/F");
...
/*        if (requireMediumTauMVA) {
    ...
    edm::Handle<reco::JetTagCollection> l2p5TausIsoOfflineNew3;
    try {iEvent.getByToken(l2p5TauJetIsoOfflineNew3Token_, l2p5TausIsoOfflineNew3);} catch (...) {;}
    ...
    if (l2p5Taus.isValid() && l2p5Tracks.isValid() && hltBeamSpot.isValid() && pv && l2p5Taus->size()>0) {
		...
		float iso_offlineNew3 = -1.f;
		if(l2p5TausIsoOfflineNew3.isValid()){
		  iso_offlineNew3 = 0;
		   for(auto const & iterIso : *l2p5TausIsoOfflineNew3){
		      float iso_dr = reco::deltaR(iterIso.first->eta(),iterIso.first->phi(),caloTau_eta,caloTau_phi);
			if(iso_dr<0.1){
			iso_offlineNew2 = iterIso.second;
		      }
		...
		//std::cout<<"Saikat Karmakar"<<"		"<<"i="<<i<<std::endl;
                l2p5TauPt = caloTauRef->pt();
	        //out<<l2p5TauPt<<std::endl;
