...
#include "TH1F.h"
class HEPTopTaggerV2_fixed_R : public edm::EDAnalyzer {
   public:
      HEPTopTaggerV2_fixed_R(const edm::ParameterSet &);
...
HEPTopTaggerV2_fixed_R::HEPTopTaggerV2_fixed_R(const edm::ParameterSet& cfg) :

   src_( cfg.getUntrackedParameter<edm::InputTag>( "src" ) ){
   edm::Service<TFileService> fs;
   h1 = fs->make<TH1F>("one","one",1,0.,2.);
   h2 = fs->make<TH1F>("two","two",1,0.,2.);
}
